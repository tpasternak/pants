...
impl Store {
  ...
  pub fn local_only<P: AsRef<Path>>(
    ...
    Ok(Store {
      local: local::ByteStore::new(io_pool, path)?,
      remote: None,
    })
  }
...
mod local {
  ...
  pub mod tests {
    ...
    pub fn new_store<P: AsRef<Path>>(dir: P) -> ByteStore {
      ByteStore::new(futures_cpupool::CpuPool::new_num_cpus(), dir).unwrap()
    }
...
mod tests {
  ...
  fn new_local_store<P: AsRef<Path>>(dir: P) -> Store {
    Store::local_only(futures_cpupool::CpuPool::new_num_cpus(), dir)
